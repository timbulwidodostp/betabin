...
{hi:help zib postestimation}{right: ({browse "http://www.stata-journal.com/article.html?article=up0058":SJ18-1: st0337_1})}
{hline}

{title:Title}

{p2colset 5 27 29 2}{...}
{p2col :{hi:zib postestimation} {hline 2}}Postestimation tools for zib{p_end}
{p2colreset}{...}
...
{pstd}
The following postestimation commands are available after {cmd:zib}:
...
{synopt :{helpb zib postestimation##predict:predict}}predictions, residuals,
...
specified; or 
{bind:(1-p_j)*N*(exp(x_j b) * exposure_j})/{1+exp(x_j b)*exposure_j} if 
{cmd:exposure()} was specified.
...
{phang2}{cmd:. zib yo x1, inflate(z1 z2) n(n)}{p_end}
...
{p 7 14 2}Help:  {helpb zib} (if installed){p_end}
