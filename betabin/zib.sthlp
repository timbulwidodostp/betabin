...
{hi:help zib}{right: ({browse "http://www.stata-journal.com/article.html?article=up0058":SJ18-1: st0337_1})}
{hline}

{title:Title}

{p2colset 5 12 14 2}{...}
{p2col :{hi:zib} {hline 2}}Zero-inflated binomial regression{p_end}
{p2colreset}{...}
...
{p 8 15 2}
{cmd:zib} {depvar} [{indepvars}] {ifin} {weight}{cmd:,}{break}
...
{synopt :{it:{help zib##maximize_options:maximize_options}}}control the maximization process; seldom used{p_end}
...
{help weight}.{p_end}
{p 4 6 2}See {help zib postestimation} for features
...
{pstd}
{cmd:zib} estimates a zero-inflated binomial (ZIB) regression of {it:depvar}
on {it:indepvars}, where {it:depvar} is a nonnegative count variable.
...
{phang2}{cmd:. generate y  = rbinomial(n, p)}{p_end}
{phang2}{cmd:. generate yo = y*z}{p_end}

{pstd}Fit zero-inflated binomial model{p_end}
{phang2}{cmd:. zib yo x1, inflate(z1 z2) n(n) link(logit) ilink(probit)}{p_end}

{pstd}Replay results, displaying odds ratios{p_end}
{phang2}{cmd:. zib, eform}{p_end}
...
{pstd}
{cmd:zib} stores the following in {cmd:e()}:
...
{synopt:{cmd:e(cmd)}}{cmd:zib}{p_end}
...
{p 7 14 2}Help:  {help zib postestimation} (if installed), {manhelp nbreg R}, 
{manhelp poisson R}, {manhelp svy_estimation SVY:svy estimation},
{manhelp tnbreg R}, {manhelp tpoisson R}, {manhelp xtpoisson XT},
{manhelp zinb R}{p_end}
